...
const ProjectCalendar = ({ tasks }: ProjectCalendarProps) => {
    ...
    const getTasksForDate = (date: Date) => tasks.filter((task) => task.due_date && isSameDay(new Date(task.due_date), date));
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="not-dark:bg-white dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex gap-2 items-center">
                            ...
                            <span className="text-zinc-900 dark:text-white text-sm font-semibold">{format(currentMonth, "MMMM yyyy")}</span>
                    ...
                    <div className="grid grid-cols-7 text-xs text-zinc-600 dark:text-zinc-400 mb-2 text-center font-medium">
                        ...
                        {daysInMonth.map((day) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {dayTasks.length > 0 && (
                                        <span className="text-[10px] text-blue-700 dark:text-blue-400 font-medium">{dayTasks.length} tasks</span>
                                    )}
                                </button>
                            );
                        })}
                            ...
                            {getTasksForDate(selectedDate).map((task) => (
                                ...
                                >
                                    <div className="flex justify-between mb-2">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${typeColors[task.type] || 'bg-zinc-200 text-zinc-800'}`}>
                                            {task.type}
                                        </span>
                                    </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-zinc-950 dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                    <h3 className="text-zinc-900 dark:text-white text-sm font-semibold flex items-center gap-2 mb-3">
                            ...
                            {upcomingTasks.map((task) => (
                                ...
                                >
                                    <div className="flex justify-between items-start text-sm">
                                        ...
                                        <span className={`text-xs px-2 py-0.5 rounded font-medium ${typeColors[task.type] || 'bg-zinc-200'}`}>
                                            {task.type}
                                        </span>
                                    </div>
                                    <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-1">{task.due_date ? format(new Date(task.due_date), "MMM d") : ""}</p>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                {overdueTasks.length > 0 && (
                    <div className="bg-white dark:bg-zinc-950 border border-red-300 dark:border-red-500 border-l-4 rounded-lg p-4">
                        <h3 className="text-red-700 dark:text-red-400 text-sm font-semibold flex items-center gap-2 mb-3">
                        ...
                        <div className="space-y-2">
                            {overdueTasks.slice(0, 5).map((task) => (
                                <div key={task.id} className="bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 p-3 rounded-lg transition" >
                                    <div className="flex justify-between text-sm text-zinc-900 dark:text-white font-medium">
                                        <span>{task.title}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded bg-red-200 dark:bg-red-500 text-red-900 dark:text-red-950 font-medium`}>
                                            {task.type}
                                        </span>
                                    </div>
