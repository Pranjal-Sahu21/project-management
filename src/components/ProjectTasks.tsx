...
const priorityTexts: Record<string, { background: string; prioritycolor: string }> = {
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-4 mb-4">
                {["status", "type", "priority", "assignee"].map((name) => {
                    const options: Record<string, { label: string; value: string }[]> = {
                        ...
                        assignee: [
                            ...
                            ...assigneeList.map((n: any) => ({ label: n, value: n })),
                        ],
                    };
                    return (
                ...
                {(filters.status || filters.type || filters.priority || filters.assignee) && (
                    <button type="button" onClick={() => setFilters({ status: "", type: "", priority: "", assignee: "" })} className="px-3 py-1 flex items-center gap-2 rounded bg-gradient-to-br from-purple-400 to-purple-500 text-zinc-100 dark:text-zinc-200 text-sm transition-colors cursor-pointer" >
                        <XIcon className="size-3" /> Reset
                    </button>
                )}

                {selectedTasks.length > 0 && (
                    <button type="button" onClick={handleDelete} className="px-3 py-1 flex items-center gap-2 rounded bg-gradient-to-br from-indigo-400 to-indigo-500 text-zinc-100 dark:text-zinc-200 text-sm transition-colors cursor-pointer" >
                            ...
                            <thead className="text-xs uppercase dark:bg-zinc-800/70 text-zinc-500 dark:text-zinc-400 ">
                                <tr>
                                    <th className="pl-2 pr-1">
                                        <input onChange={() => selectedTasks.length === filteredTasks.length && filteredTasks.length > 0 ? setSelectedTasks([]) : setSelectedTasks(filteredTasks.map((t) => t.id))} checked={filteredTasks.length > 0 && selectedTasks.length === filteredTasks.length} type="checkbox" className="size-3 accent-zinc-600 dark:accent-zinc-500" />
                                    </th>
                                    ...
                                    filteredTasks.map((task) => {
                                        ...
                                        return (
                                            <tr key={task.id} onClick={() => router.push(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className=" border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" >
                                                ...
                                                <td className="px-4 py-2">
                                                    <span className={`text-xs px-2 py-1 rounded font-semibold ${background} ${prioritycolor}`}>
                                                        {task.priority}
                                                    </span>
                                                </td>
